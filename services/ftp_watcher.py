    ...
    with app.app_context():
        ...
        for device in devices:
            ...
            for csv_file in csv_files:
                ...
                future = executor.submit(_process_csv_file, csv_file, device, app)
                try:
                    future.result(timeout=FILE_PROCESS_TIMEOUT)
                except FutureTimeoutError:
                    logger.error(f"Timeout processing {csv_file} for device {device.device_id} (>{FILE_PROCESS_TIMEOUT}s), skipping")
                finally:
